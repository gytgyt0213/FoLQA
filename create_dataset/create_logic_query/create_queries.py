...
def index_dataset(dataset_name, force=False):
    ...
    base_path = '../KG/{0}/'.format(dataset_name)
    files = ['train.txt']
...
def write_links(dataset, ent_out, small_ent_out, max_ans_num, name):
    ...
    output_dir = os.path.join('../create_dataset', dataset)
    os.makedirs(output_dir, exist_ok=True)
    
    with open(os.path.join(output_dir, '%s-queries.pkl'%name), 'wb') as f:
        pickle.dump(queries, f)
    with open(os.path.join(output_dir, '%s-tp-answers.pkl'%name), 'wb') as f:
        pickle.dump(tp_answers, f)
    with open(os.path.join(output_dir, '%s-fn-answers.pkl'%name), 'wb') as f:
        pickle.dump(fn_answers, f)
    with open(os.path.join(output_dir, '%s-fp-answers.pkl'%name), 'wb') as f:
...
def ground_queries(dataset, query_structure, ent_in, ent_out, small_ent_in, small_ent_out, gen_num, max_ans_num, query_name, mode, ent2id, rel2id):
    ...
    with open(os.path.join(output_dir, '%s-queries.pkl'%name_to_save), 'wb') as f:
        pickle.dump(queries, f)
    with open(os.path.join(output_dir, '%s-fp-answers.pkl'%name_to_save), 'wb') as f:
        pickle.dump(fp_answers, f)
    with open(os.path.join(output_dir, '%s-fn-answers.pkl'%name_to_save), 'wb') as f:
        pickle.dump(fn_answers, f)
    with open(os.path.join(output_dir, '%s-tp-answers.pkl'%name_to_save), 'wb') as f:
        pickle.dump(tp_answers, f)
...
def generate_queries(dataset, query_structures, gen_num, max_ans_num, gen_train, gen_valid, gen_test, query_names, save_name):
    ...
    log_dir = os.path.join('../create_dataset', dataset)
    os.makedirs(log_dir, exist_ok=True)
    set_logger(log_dir + "/", name_to_save)
    ...
    if gen_train:
        if gen_train:
            train_queries, train_tp_answers, train_fp_answers, train_fn_answers = \
...
def fill_query(query_structure, ent_in, ent_out, answer, ent2id, rel2id):
    ...
    if all_relation_flag:
        r = -1
        if (answer not in ent_in) or (len(ent_in[answer]) == 0):
            return True
        ...
        for i in range(len(query_structure[-1]))[::-1]:
            ...
            rel_cands = list(ent_in[answer].keys()) if (answer in ent_in) else []
            if not rel_cands:
                return True

            found = False
            for _ in range(40):
                r_tmp = random.choice(rel_cands)
            ...
            if not found:
                return True
            tails = ent_in[answer].get(r, set())
            if not tails:
                return True

            query_structure[-1][i] = r
            answer = random.sample(list(tails), 1)[0]
...
@click.command()
@click.option('--dataset', default="WN18RR")
@click.option('--seed', default=0)
@click.option('--gen_train_num', default=6000)
...
@click.option('--gen_id', default= 17)# 1 2 5 6 7 8 10 14 15 16; 2i 3, 2u 13, 2nu 17
...
def main(dataset, seed, gen_train_num, gen_valid_num, gen_test_num, max_ans_num, reindex, gen_train, gen_valid, gen_test, gen_id, save_name, index_only):
    train_num_dict = {'FB15k': 273710, "FB15k-237": 149689, "NELL": 107982, "WN18RR":107982}
    valid_num_dict = {'FB15k': 8000, "FB15k-237": 5000, "NELL": 4000, "WN18RR":4000}
    test_num_dict = {'FB15k': 8000, "FB15k-237": 5000, "NELL": 4000, "WN18RR":4000}
    if gen_train and gen_train_num == 0:
        ...
        elif 'WN18RR' in dataset:
            gen_train_num = 50_000
        else:
    ...
    query_names = ['1p', '2p', '3p', '2i', '3i', 'pi', '2pi', 'ip', '2in', '3in', 'pin', 'pni', 'inp', '2u', 'up', '2pu', 'pu',
                   'nu']
