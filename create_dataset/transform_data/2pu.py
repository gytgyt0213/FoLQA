...
def parse_2pu_sample(q: Any) -> Tuple[Tuple[int, Tuple[int, int]], Tuple[int, Tuple[int, int]]]:
    assert isinstance(q, tuple) and len(q) == 3, f"2pu sample must be 3-tuple, got: {q}"
    a, b, marker = q
    ...
    if len(a[1]) != 2 or len(b[1]) != 2:
        raise ValueError(f"Not a 2pu sample (both branches must be 2-hop): {q}")
...
def main():
    query_path = "../FB15k/train-2pu-queries.pkl"
    answer_path = "../FB15k/train-2pu-tp-answers.pkl"
    out_path = "../FB15k/transformed_answers/2pu.json"


    ap = argparse.ArgumentParser(description="Convert 2PU (2-hop ∪ 2-hop) queries+answers PKL to JSON list.")
    ap.add_argument("--queries_pkl", default=query_path, help="Path to 2pu *-queries.pkl")
    ap.add_argument("--answers_pkl", default=answer_path, help="Path to 2pu *-answers.pkl (tp/fp/fn all OK)")
    ...
    total = ok = skipped_shape = skipped_not_2pu = skipped_no_ans = 0
    ...
    for _, query_set in queries_obj.items():
        ...
        for q_raw in query_iter:
            ...
            try:
                (e1, (r1, r2)), (e2, (r3, r4)) = parse_2pu_sample(q)
            except Exception:
                skipped_not_2pu += 1
                continue
    ...
    print(f"Total: {total}, OK: {ok}, Skipped(shape): {skipped_shape}, Skipped(not-2pu): {skipped_not_2pu}, Skipped(no-ans): {skipped_no_ans}")
